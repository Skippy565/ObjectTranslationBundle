...
class ObjectTranslationTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testTranslateObjectToObject()
    {
        ...
        $this->assertArraySubset(
            ['status' => 'true', 'message' => 'Mapped'],
            $result['result'],
            true,
            'Result is missing status or message key.'
        );
        ...
        foreach ($translationObject->translationModel as $key => $value) {
            if ($value != 'noOverride') {
                $this->assertEquals(
                    $fromObject->__get($key),
                    $data->__get($value),
                    "From object $key does not match to object $value"
                );
            }
        }
        ...
        foreach ($translationObject->mappingFunctions as $property => $method) {
            $this->assertObjectHasAttribute(
                $property,
                $translationObject->toObject,
                "Property $property not found in to object"
            );
            $this->assertEquals(
                $fromObject->__get('firstName').' '.$fromObject->__get('lastName'),
                $data->__get($property)
            );
        }
    }
    ...
    public function testTranslateObjectToArray()
    {
        ...
        foreach ($translationObject->mappingFunctions as $property => $method) {
            $this->assertTrue(
                isset($translationObject->toObject[$property]),
                "Property $property not found in to object"
            );
            $this->assertEquals(
                $fromObject->__get('firstName').' '.$fromObject->__get('lastName'),
    ...
    public function testTranslateArrayToArray()
    {
        ...
        // verify translation data
        $this->assertTrue(is_array($data), 'Incorrect data type. Expected array, received '.gettype($data).'.');
     ...
     * @covers AbstractObjectTranslation::translate
     */
    public function testTranslateArrayToObject()
    {
        // instantiate from object
        $fromObject = json_decode(json_encode(new FromTestObject()), true);
        ...
        // verify translation data
        $this->assertTrue(is_object($data), 'Incorrect data type. Expected object, received '.gettype($data).'.');
    ...
    public function testBuilderPattern() {
        ...
        // instantiate translation object
        $translationObject = new TranslationTestObject($fromObject, $toObject);
        $translationObject->translationModel = [];
        $translationObject->mappingFunctions = [];
        $translationObject->overwritingRules = [];

        $translationObject->addToTranslationModel('email', 'email')
                    ->addToTranslationModel('email1', 'noOverride')
                    ->addToTranslationModel('email2', 'willoverride')
                    ->addToMappingFunctions('fullName', 'mapName')
                    ->addToOverwritingRules('noOverride', [null])
                    ->addToOverwritingRules('willOverride', ['Change']);
